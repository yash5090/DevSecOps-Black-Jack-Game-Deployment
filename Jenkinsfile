pipeline {
    ...
    stages {
        ...
        stage('Checkout from Git') {                        
            steps {                                       
                git branch: 'main', url: 'https://github.com/yash5090/DevSecOps-Black-Jack-Game-Deployment.git'
            }
        }
        ...
        stage("Docker Image Tagging"){
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                            sh "docker tag blackjack yash5090/blackjack:latest " 
                        //}
                    }
                }
            }
        ...
        stage('Docker Image Scanning') { 
            steps { 
                sh "trivy image --format table -o trivy-image-report.html yash5090/blackjack:latest" 
            } 
        } 
        ...
        stage("Image Push to DockerHub") {
            steps{
                script{
                    //dir('Band Website') {
                        withDockerRegistry(credentialsId: 'docker', toolName: 'docker') {
                            sh "docker push yash5090/blackjack:latest "
                        //}
                    }
                }
            }
        ...
        stage('Docker Scout Image Scanning') {
            steps {
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                       sh 'docker-scout quickview yash5090/blackjack:latest'
                       sh 'docker-scout cves yash5090/blackjack:latest'
                       sh 'docker-scout recommendations yash5090/blackjack:latest'
                       sh 'docker-scout attestation yash5090/blackjack:latest'
                   }
                }   
            }
        }
        ...
        stage("TRIVY"){
            steps{
                //dir('Band Website') {
                    sh "trivy image yash5090/blackjack:latest > trivyimage.txt"   
                //}
            }
        }
        ...
        stage('Docker Scout Artifacts Analysis') {
            steps {
                script{
                  withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){
                      sh 'docker-scout sbom --format list yash5090/blackjack:latest' // docker-scout sbom yash5090/blackjack:latest
                    }
                }   
            }
        }
        ...
        stage ("Remove Docker Container") {
            steps{
                sh "docker stop blackjack | true"
                sh "docker rm blackjack | true"
             }
        }
        ...
        stage('Deploy to Docker Container'){
            steps{
                //dir('BMI Calculator (JS)') {
                    sh 'docker run -d --name blackjack -p 5000:80 yash5090/blackjack:latest' 
                //}
            }
        }
